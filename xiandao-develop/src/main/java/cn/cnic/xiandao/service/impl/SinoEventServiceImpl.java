...
public class SinoEventServiceImpl extends ServiceImpl<ExhibitSinoEventMapper, ExhibitSinoEvent> {
    ...
    public UpdateWrapper<ExhibitSinoEvent> noticeAndmodifyStatus(int id, String noticeMethod,  String describeEvent,String noticeTime) throws ParseException {
        UpdateWrapper<ExhibitSinoEvent> updateWrapper = new UpdateWrapper<>();
        //String time = noticeTime.toString();
        //new SimpleDateFormat("EEE MMM dd HH:mm:ss zzz yyyy", Locale.US).parse(time);
    ...
    public UpdateWrapper<ExhibitSinoEvent> modifyStatus(int id) throws ParseException {
        ...
        System.out.println(sdf2.format(sdf1.parse(resolveTime)));
